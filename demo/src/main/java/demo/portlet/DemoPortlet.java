...
import demo.constants.DemoPortletKeys;
...
public class DemoPortlet extends MVCPortlet {
	...
	public void render(RenderRequest renderRequest, RenderResponse renderResponse)
		...
		try 
		{
			...
			{  
			System.out.println("Hi"+themeDisplay.getUser().getFullName());
			
			ServiceContext serviceContext = ServiceContextFactory.getInstance(renderRequest);;
			System.out.println("site name : "+themeDisplay.getScopeGroup().getName(Locale.ENGLISH));
			//updateName(renderRequest,renderResponse);
			updateProfilePic(renderRequest,renderResponse);
			//updateUserInfo(renderRequest,renderResponse);
	...
	private void updateName(RenderRequest renderRequest, RenderResponse renderResponse) {
		try
		{
		ThemeDisplay themeDisplay = (ThemeDisplay)renderRequest.getAttribute(WebKeys.THEME_DISPLAY);
		if(themeDisplay.isSignedIn())
		{
			...
			for(User userInfo : userData)
			{
				...
				for(activeemployeeinfosample activeemployeeinfoData : activeemployeeinfosamples)
				{
					...
					System.out.println("Firstname : "+ activeemployeeinfoData.getFirstName());

					System.out.println("Lastname : "+ activeemployeeinfoData.getLastName());
					
					user.setFirstName(activeemployeeinfoData.getFirstName());
					user.setLastName(activeemployeeinfoData.getLastName());
					userLocalService.updateUser(user);
					
					System.out.println("Updating Gender");

					Contact contact = ContactLocalServiceUtil.getContact(contactid);
					if(activeemployeeinfoData.getGender().equalsIgnoreCase("M"))
					{
						contact.setMale(true);
					}
					else if(activeemployeeinfoData.getGender().equalsIgnoreCase("F"))
					{
	...
	private void updateProfilePic(RenderRequest renderRequest, RenderResponse renderResponse) {
		try
		{
		ThemeDisplay themeDisplay = (ThemeDisplay)renderRequest.getAttribute(WebKeys.THEME_DISPLAY);
		if(themeDisplay.isSignedIn())
		{
			...
			for(User userInfo : userData)
			{
				...
				for(activeemployeeinfosample activeemployeeinfoData : activeemployeeinfosamples)
				{
					...
					if(activeemployeeinfoData.getPhoto()!=null && !activeemployeeinfoData.getPhoto().equalsIgnoreCase(""))
					{
						try
						{
						System.out.println("Updating profile pic");
						String imageName = activeemployeeinfoData.getPhoto();
						BufferedImage bufferedImage = ImageIO.read(new File("/apps/LiferayLCS/liferay-dxp-digital-enterprise-7.0-sp1/tomcat-8.0.32/webapps/ProfileImages/"+imageName));
						ByteArrayOutputStream os = new ByteArrayOutputStream();
						  
						int index=imageName.lastIndexOf('.');
						    
						ImageIO.write(bufferedImage, imageName.substring(index+1,imageName.length()), os);
						InputStream is = new ByteArrayInputStream(os.toByteArray());
						byte[] bytes = FileUtil.getBytes(is);
						userLocalService.updatePortrait(userInfo.getUserId(), bytes);
						is.close();
						os.close();
						}
						catch (Exception e) {
		...
		if(themeDisplay.isSignedIn())
		{
			...
			for(User userInfo : userData)
			{
				...
				for(activeemployeeinfosample activeemployeeinfoData : activeemployeeinfosamples)
				{
					...
					if(activeemployeeinfoData.getBirthday()!=null && !activeemployeeinfoData.getBirthday().equalsIgnoreCase(""))
					{
					Contact contact = ContactLocalServiceUtil.getContact(contactid);
					contact.setBirthday(new SimpleDateFormat("MM/dd/yyyy").parse(new SimpleDateFormat("MM/dd/yyyy").format(new SimpleDateFormat("dd-MMM-yyyy").parse(activeemployeeinfoData.getBirthday()))));
					ContactLocalServiceUtil.updateContact(contact);
					}
					...
					user.getExpandoBridge().setAttribute("Anniversary",activeemployeeinfoData.getAnniversary());
					user.getExpandoBridge().setAttribute("Dateofjoining",activeemployeeinfoData.getDOJ());
